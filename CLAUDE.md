...
| + | DOCX + PDF note export — note export gains a format choice (`ExportFormat = 'md' \| 'docx' \| 'pdf'`); PDF via an offscreen, script-free `BrowserWindow` + `printToPDF` (zero deps), DOCX via the `docx` library mapping `markdown-it` tokens → Word. Both build from the same inlined Markdown as `.md` export (shared `withTitle`/`inlineAttachments`); applies to single-note and all-notes (`ExportMenu` format pills + command-palette entries). Tasks/backup unchanged. See ADR-0013 |
...
- **Host-header allowlist** (`isLoopbackHost`, `src/main/mcp/host-guard.ts`) — blocks a browser tab from POSTing via a rebound hostname. **Do NOT also enable the SDK transport's `enableDnsRebindingProtection` with a port-less `allowedHosts`**: the SDK matches the *full* Host header including the port (`127.0.0.1:51789`), so a list like `['127.0.0.1']` 403s **every** request with "Invalid Host header" — after our own check has already passed. This shipped broken in 1.6.0/1.6.1 (the connector returned zero tools). The transport is built by `createMcpTransport()` with only `enableJsonResponse: true`; our `isLoopbackHost` (which strips the port) is the host guard. Regression test: `src/main/mcp/server.transport.test.ts`.
...
**npx PATH resolution (`src/main/mcp/bridge-config.ts`):** Claude Desktop is a GUI app and does **not** inherit the user's shell PATH, so a bare `"command": "npx"` fails to spawn for anyone using nvm/Homebrew Node — the bridge silently never starts and zero tools register. The generated config is therefore built **in the main process** (`connectors:getClaudeConfig`), which resolves an **absolute** npx path (newest nvm Node → Homebrew → system) and adds a minimal `PATH` to `env`. The sandboxed renderer can't probe the filesystem, so it fetches the prebuilt config rather than assembling it. If no npx is found, the UI warns. The token sits literally in the `--header` arg (no `${VAR}` env-interpolation dependency).
...
**The SQLCipher pragma set exists in TWO places — change both or restores break**
- `initDb()` (`src/main/db/index.ts`) and `openBackupReadonly()` (`src/main/services/restore.ts`) must issue the identical cipher pragma sequence (`cipher_page_size`, `kdf_iter`, HMAC/KDF algorithms). If the parameters ever change, a backup written with the old set will fail the restore probe with the new one — add a compat path before shipping such a change.
- The restore flow ends with `app.relaunch(); app.exit(0)` — `app.exit` intentionally skips `will-quit`, so the auto-backup hook never runs against the closed handle / freshly-swapped file. Don't "fix" it to `app.quit()`.
- A backup whose `_migrations` table contains names unknown to the current build is from a **newer** Cinder and is refused (`findUnknownMigrations` in `db/migrate.ts`). Older backups are fine — pending migrations run at next boot.
